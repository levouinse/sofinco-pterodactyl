...
export GITHUB_BASE_URL=${GITHUB_BASE_URL:-"https://raw.githubusercontent.com/levouinse/sofinco-pterodactyl"}
...
welcome() {
  ...
  output ""
  output "Copyright (C) 2026, Sofinco"
  output "https://github.com/levouinse/sofinco-pterodactyl"
  output ""
...
update_repos() {
  ...
  case "$OS" in
    ...
    arch | endeavouros | artix)
      output "Updating package repositories..."
      pacman -Sy --noconfirm $args
      ;;
    gentoo)
      output "Syncing Portage tree..."
      emerge --sync
      ;;
    void)
      output "Updating package repositories..."
...
install_packages() {
  ...
  rocky | almalinux)
    ...
    ;;
  arch | endeavouros | artix)
    eval pacman -S --noconfirm $args "$1"
    ;;
  gentoo)
    eval emerge "$1"
    ;;
  void)
    eval xbps-install -y "$1"
    ;;
  fedora)
    eval dnf -y $args install "$1"
    ;;
  freebsd)
    eval pkg install -y "$1"
    ;;
  slackware)
    eval slackpkg install "$1"
    ;;
  esac
}
...
install_firewall() {
  ...
  arch | endeavouros | artix)
    output ""
    output "Installing UFW for Arch-based systems"
  ...
  gentoo)
    output ""
    output "Installing iptables for Gentoo"

    if ! [ -x "$(command -v iptables)" ]; then
      install_packages "net-firewall/iptables" true
    fi
  ...
  void)
    output ""
    output "Installing iptables for Void Linux"

    if ! [ -x "$(command -v iptables)" ]; then
      install_packages "iptables" true
    fi
  ...
  freebsd)
    output ""
    output "Using FreeBSD pf firewall"

    success "pf firewall available"

    ;;
  slackware)
    output ""
    output "Installing iptables for Slackware"
...
firewall_allow_ports() {
  ...
  freebsd)
    for port in $1; do
      echo "pass in proto tcp from any to any port $port" >> /etc/pf.conf
    done
...
freebsd)
  [ "$OS_VER_MAJOR" == "13" ] && SUPPORTED=true
  [ "$OS_VER_MAJOR" == "14" ] && SUPPORTED=true
  ;;
slackware)
  [ "$OS_VER_MAJOR" == "15" ] && SUPPORTED=true
  ;;
*)
  SUPPORTED=false
  ;;
